 <?php
session_start();
include('../assets/include/db.php');

if($_SESSION['title']!=$_POST['title']) {
$title=$_POST['title'];
}
else {
    $title=$_SESSION['title'];
}

if($_SESSION['subtitle']!=$_POST['subtitle']) {
$subtitle=$_POST['subtitle'];
}
else {
    $subtitle=$_SESSION['subtitle'];
}

if($_SESSION['comment']!=$_POST['comment']) {
    $comment=$_POST['comment'];
    }
    else {
        $subtitle=$_SESSION['comment'];
    }

if($_SESSION['description']!=$_POST['description']) {
$description=$_POST['description'];
}
else {
    $description=$_SESSION['description'];
}


$sql="UPDATE security SET title='$title', subtitle='$subtitle', comment='$comment', description='$description', datetime='$datetime' WHERE  id='$_POST[updateserviceid]'";
$run=mysqli_query($conn,$sql);
if($run){
    echo 'Done';
    header('location:../features.php?action=done');
}else{
    echo 'Error';
}

    
          ?>
