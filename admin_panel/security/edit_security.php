...
$sql="SELECT * FROM security WHERE id='$_GET[id]'";
 ...
 $datetime=$row['datetime'];
	...
	echo '<form class="form fv-plugins-bootstrap fv-plugins-framework" id="updateserviceform" novalidate="novalidate" action="integrations/update_integrations.php" method="post" enctype="multipart/form-data">
			...
			<div class="form-group ">
				...
				<input class="form-control h-auto py-4 rounded-lg " type="text" id="comment" name="comment"  value="'.$_SESSION['comment']=$comment.'">
				<div class="fv-plugins-message-container"></div>
			</div>
			<div class="form-group ">
				<label class="font-weight-bolder text-dark"> Date & Time</label>
				<input class="form-control h-auto py-4 rounded-lg " type="text" id="datetime" name="datetime"  value="'.$_SESSION['datetime']=$datetime.'">
				<div class="fv-plugins-message-container"></div>
			</div>
